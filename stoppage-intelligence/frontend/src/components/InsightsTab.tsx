...
export default function InsightsTab({ uploadId, radius }: Props) {
  ...
  useEffect(() => {
    ...
    api.get("/analytics/summary", { params: p }).then((r) => setSummary(r.data));
    api.get("/analytics/hourly", { params: { upload_id: String(uploadId) } }).then((r) => setHourly(r.data.distribution));
    api.get("/analytics/poi-breakdown", { params: p }).then((r) => setPoiBreakdown(r.data.breakdown));
    api.get("/analytics/route-breakdown", { params: { upload_id: String(uploadId), limit: "12" } }).then((r) => setTopRoutes(r.data.routes));
    api.get("/analytics/top-clusters", { params: { ...p, limit: "10" } }).then((r) => setTopClusters(r.data.clusters));
    api.get("/analytics/top-clusters", { params: { ...p, classification: "unauthorized", limit: "10" } }).then((r) => setUnauthorized(r.data.clusters));
  }, [uploadId, radius]);
      ...
      <div className="charts-grid" style={{ padding: 0, marginBottom: 20 }}>
        <div className="panel" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={piData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
        ...
        <div className="panel" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={hourly}>
              ...
              <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 6 }} labelFormatter={(h) => `${h}:00`} />
        ...
        <div className="panel" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={poiBreakdown.slice(0, 10)} layout="vertical">
              <XAxis type="number" tick={{ fill: "#8b949e", fontSize: 11 }} />
              <YAxis type="category" dataKey="poi_type" tick={{ fill: "#8b949e", fontSize: 10 }} width={110} />
        ...
        <div className="panel" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={topRoutes.slice(0, 10)} layout="vertical">
              <XAxis type="number" tick={{ fill: "#8b949e", fontSize: 11 }} />
              <YAxis type="category" dataKey="route_code" tick={{ fill: "#8b949e", fontSize: 10 }} width={70} />
              <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 6 }} />
